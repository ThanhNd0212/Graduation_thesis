...
process(session_id, message, reply_to_msg_id) -> response dict (chatbot_demo_plan.md §2).
...
# Vietnamese filler/conjunction words that NER sometimes mis-tags as PRODUCT_NAME (§3 NER quirk)
...
class ChatPipeline:
    ...
    def process(self, session_id: str, message: str, reply_to_msg_id: str | None = None,
        ...
        # Drop such entities when their value exactly matches the selection digit (§B1 fix).
        ...
        # give_product is ambiguous → borrow the last meaningful intent (§4.6)
        eff = set(intents)
        ...
        if 'agree_order' in intents:
            s.agreed = True
        # ── A.0) Reset completed order on new greeting (§ ghost-order fix) ──────
        ...
        # ── A.0-done) Khóa mọi nhánh khi đơn đã hoàn tất ─────────────────────
        ...
        # ── A.1) Auto-confirm a product just availability-checked (§ ghost-recap fix) ──
        ...
        # ── A.3) finalize prompt pending (§6 double-confirm fix) ───────────────
        ...
        # ── A.4) Xác nhận hủy giỏ sau khi hỏi lại ý định ────────────────────
        ...
        # ── A.5) Cancel / exit order flow (hỏi xác nhận trước khi xóa giỏ) ──────
        ...
        # ── B) order-stage continuation (R3/§5) ────────────────────────────────
        ...
        elif action is None and s.order_stage == 'await_pickup':   # §4.10 pickup confirm
            if _affirmative(message, intents):
        ...
        # ── C) customer_reject after a proposal (§4.2) ─────────────────────────
        ...
        # ── D) proposal confirmation (§3/§4/§4.1) ──────────────────────────────
        if action is None:
            target = None
            if reply_to_msg_id:
                prop = s.get_proposal(reply_to_msg_id)
            ...
            if target:
                ...
                if status == 'chosen':
                    ...
                    if 'ask_product_availability' in prop.get('origin', []) and \
                        ...
                        # Hỏi giá từ danh sách: cập nhật last_product, KHÔNG thêm cart
                        s.last_product_id = product.get('product_id')
                        lp = self._last_product(s)
                        action, payload = ('product_info', lp) if lp else ('need_product', None)
        ...
        if action is None:
            ...
            elif 'get_product_direct' in eff:            # §4.10
        ...
        # ── J) escalation: 3 consecutive turns the bot couldn't handle (§7) ────
