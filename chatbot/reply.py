...
# business constants (dialogue_rules.md )
SHIP_FEE = 20000
GIFT_FEE = 15000
DELIVERY = '2–3 ngày'
...
def render(state, intents, entities, action, payload) -> str:
    ...
    if action == 'confirmed':
        ...
        if [m for m in ('NAME', 'PHONE', 'ADDRESS') if m not in state.customer]:
            msg += '\n' + CUSTOMER_FORM
        return msg
    if action == 'suggest_confirm':        #  — show brief info after suggest selection
        p = payload
        extra = ''
        if p.get('category'):
            extra += f' — {p["category"]}'
    ...
    if action == 'product_image':          #  (no image URLs in DB — simulated)
    ...
    if action == 'complaint':
        return COMPLAINT
    if action == 'buy_thanks':
        return BUY_THANKS
    if action == 'shop_info':              #  (ask_shop_info)
        return SHOP_INFO
    if action == 'get_direct_recap':       #  (order confirmed for pickup)
    ...
    if action == 'get_direct_ask':         #  (pickup intent, not yet confirmed)
    ...
    if action == 'order_done_reminder':    #  — guard when stage=done, blocks all other intents
    ...
    if action == 'order_done':             #  — response differs by payment method
        if state.payment == 'chuyển khoản':
